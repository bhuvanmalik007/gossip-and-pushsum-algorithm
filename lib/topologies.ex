  ...
  def line(numNodes) do
    for i <- 1..numNodes do
      neighboursList =
        ...
        spawn(fn -> NodeGenServer.start_link(i, neighboursList) end)
    end
    # IO.puts("\nregistry keys: #{inspect(Registry.keys(:node_store, self()))}")
    # Process.sleep(200)
    convergence_task = Task.async(fn -> Topologies.converging(numNodes) end)
        :global.register_name(:mainproc,convergence_task.pid)
        start_time = System.system_time(:millisecond)
        rand_pid = NodeGenServer.pidRetriever(Enum.random(1..numNodes))
...
convergence_task = Task.async(fn -> Topologies.converging(numNodes) end)
  ...
  def start() do
    ...
    # spawn(Topologies, :line, [100])
    # spawn(Topologies, :full, [10])
    # spawn(Topologies, :imp_line, [10])
